...
const expDate = (numer) => {
    ...
    // Buat objek Date dari input dan tambahkan 30 hari
    let tgl = new Date(numer);
    tgl.setDate(tgl.getDate() + 30); // tambah 30 hari

    const day = tgl.getDate();
    const month = tgl.getMonth();
    const thisDay = tgl.getDay(); // ambil hari dari tanggal baru
    const year = tgl.getFullYear(); // gunakan getFullYear untuk tahun

    return `${myDays[thisDay]}, ${day} - ${myMonths[month]} - ${year}`;
}
const tanggal = (numer) => {
...
module.exports = { unixTimestampSeconds, generateMessageTag, processTime, webApi, getRandom, getBuffer, fetchJson, runtime, clockString, sleep, isUrl, getTime, formatDate, tanggal, formatp, jsonformat, reSize, toHD, logic, generateProfilePicture, bytesToSize, checkBandwidth, getSizeMedia, parseMention, getGroupAdmins, readFileTxt, readFileJson, getHashedPassword, generateAuthToken, cekMenfes, generateToken, batasiTeks, randomText, isEmoji, getTypeUrlMedia, pickRandom, getAllHTML, toIDR, expDate };
